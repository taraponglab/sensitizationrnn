...
from tensorflow.keras.layers import Dense, Bidirectional, LSTM, Dropout, SimpleRNN, GRU
...
def train_rnn_model(x_train, x_test, y_train, y_test, model_filename='rnn.keras', plot_filename='training_lost.png', epochs=50, batch_size=2):
    ...
    # Build BiGRU model
    model = Sequential()
    model.add(Bidirectional(GRU(units=64, input_shape=(1, num_features), return_sequences=True)))
...
def main():
    # Load data from CSV
    x_train = pd.read_csv("sensitization_x_train_augmented_smiles_token_rdkit.csv",index_col=0)
    x_test = pd.read_csv("sensitization_x_test_augmented_smiles_token_rdkit.csv",index_col=0)
    y_train = pd.read_csv("sensitization_y_train_augmented_smiles.csv",index_col=0)
    y_test = pd.read_csv("sensitization_y_test_augmented_smiles.csv",index_col=0)
    accuracy_train, accuracy_test,mcc_train,mcc_test,precision_train,precision_test,recall_train,recall_test, specificity_train,specificity_test, roc_auc_test,roc_auc_train, model_file, plot_file = train_rnn_model(x_train, x_test, y_train, y_test)
