...
def train_bilstm_model(x_train, x_test, y_train, y_test, model_filename='bilstm.keras', plot_filename='training_lost.png', epochs=50, batch_size=2):
...
def main():
    # Load data from CSV
    x_train = pd.read_csv("sensitization_x_train_augmented_smiles_rdkit.csv",index_col=0)
    x_test = pd.read_csv("sensitization_x_test_augmented_smiles_rdkit.csv",index_col=0)
    ...
    accuracy_train, accuracy_test,mcc_train,mcc_test,precision_train,precision_test,recall_train,recall_test, specificity_train,specificity_test, roc_auc_test,roc_auc_train, model_file, plot_file = train_bilstm_model(x_train, x_test, y_train, y_test)
